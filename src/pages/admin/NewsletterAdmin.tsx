...
import { Users, Send, Trash2, Download, CheckCircle, Clock, XCircle, Loader2, AlertCircle } from 'lucide-react';
import { api } from '../../api';
...
export default function NewsletterAdmin() {
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        setSendResult(null);
        try {
            const res = await api.sendNewsletter({ subject, body });
                ...
                {([['subscribers', 'Iscritti', <Users size={15}/>], ['send', 'Invia Newsletter', <Send size={15}/>]] as const).map(([id, label, icon]) => (
                    <button
                        ...
                        className={`flex items-center gap-2 px-5 py-3 text-sm font-medium border-b-2 transition-colors ${
                            tab === id
                                ? 'border-dis-green text-dis-green'
                        ...
                        <form onSubmit={handleSend} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-zinc-400 text-xs uppercase tracking-wider mb-1.5">
                                    Corpo (testo, HTML o Markdown renderizzato)
                                </label>
                                <textarea
                                    value={body}
                                    ...
                                    required
                                    rows={14}
                                    className="w-full bg-zinc-800 border border-zinc-700 text-white placeholder-zinc-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-dis-green transition-colors resize-y font-mono"
                                />
                            </div>
                            <button
                                type="submit"
