...
interface PortfolioShowcaseProps {
  items: PortfolioItem[];
  title: string;
  enabledItemIds?: number[]; // Nuova prop per gli ID degli elementi abilitati
}

const PortfolioShowcase: React.FC<PortfolioShowcaseProps> = ({ items, title, enabledItemIds }) => {
                ...
                <button
                  ...
                  className={`w-full text-left p-4 rounded-lg transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500/50 transform group ${
                    selectedItem?.id === item.id
                      ...
                      : enabledItemIds && enabledItemIds.includes(item.id) // Se è abilitato
                  ...
                  disabled={enabledItemIds && !enabledItemIds.includes(item.id)} // Disabilita se non è nell'array degli ID abilitati
                >
                  <h3 className={`font-semibold text-lg transition-colors duration-300 ${ selectedItem?.id === item.id ? 'text-green-300' : (enabledItemIds && enabledItemIds.includes(item.id) ? 'text-white group-hover:text-green-300' : 'text-gray-500')}`}>{item.title}</h3>
                  <p className={`text-sm line-clamp-2 mt-1 ${selectedItem?.id === item.id ? 'text-gray-300' : (enabledItemIds && enabledItemIds.includes(item.id) ? 'text-gray-400 group-hover:text-gray-300' : 'text-gray-500')}`}>{item.summary}</p>
                </button>
              </li>
            ))}
          </ul>
